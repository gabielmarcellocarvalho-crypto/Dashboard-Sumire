...
/* eslint-disable react-hooks/refs, react-hooks/purity, react-hooks/set-state-in-effect,
   ...
   componente vendorizado (copy-paste de terceiros, ver conversa com o operador):
   acesso síncrono a pathRef.current é intencional (mede o path SVG já commitado
   pra animar segmentos do gráfico), os `setMounted(true)` em efeito são o padrão
   usual de "montado no client" pra portals/SSR, e os `any`/generics não usados
   vêm de tipos do d3/visx que o próprio componente original já trazia assim. */
